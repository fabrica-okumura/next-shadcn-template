...
const PREFIXES_TO_STRIP = ['--master-', '--color-', '--radius-', '--spacing-', '--typography-']

function stripVariablePrefixes(cssText) {
  return PREFIXES_TO_STRIP.reduce((acc, prefix) => acc.replaceAll(prefix, '--'), cssText)
}
...
function postProcessCss() {
  ...
  const processed = stripVariablePrefixes(shifted) // カスタムプレフィックスを '--' に統一

  fs.writeFileSync(OUTPUT, processed, 'utf8') // 処理後の CSS を保存
}

try {
  postProcessCss() // CSS の後処理
  console.log('デザイントークン CSS の後処理が完了しました。') // 正常終了メッセージ
} catch (error) {
